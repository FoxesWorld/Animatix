...
public class TextSplitter {
    ...
    public static void setInitialPositions(List<JLabel> labels, int startX, int startY, int spacing) {
        ...
        if (font != null) {
            fontMetrics = tempLabel.getFontMetrics(font);
        }

        for (JLabel label : labels) {
            if (fontMetrics != null) {
                int labelWidth = fontMetrics.stringWidth(label.getText()); // Получаем ширину текста
                label.setBounds(currentX, startY, labelWidth, fontMetrics.getHeight()); // Устанавливаем размеры и позицию
                currentX += labelWidth + spacing; // Сдвигаем X с учетом ширины текста и отступа
            }
        }
    }
